...
import numpy as np
from locomotion.utils.interplation import cubicBezier, cubicBezierFirstDerivative, cubicBezierSecondDerivative
...
class FootSwingTrajectory:
    def __init__(self):
        ...
        self._p:np.ndarray = np.zeros((3,1), dtype=DTYPE)
        self._v:np.ndarray = np.zeros((3,1), dtype=DTYPE)
        self._a:np.ndarray = np.zeros((3,1), dtype=DTYPE)
        # float or int
        self._height = 0.0
